...
from spe.cust_ind import PDIMDIHL,hhpc,pchannel


class LMR6(bt.Strategy):
    params = (
        ('trade_size', 1),
        ('benchmark_name', 'NSEI'),
        ('tick_size', 0.0005),
    ...
    def next(self):
        ...
        for d in self.datas:
            ...
            buy_cond = self.inds[d]['pdi'][0] < self.inds[d]['mdi'][0] and self.inds[d]['adx'][0] > self.inds[d]['adxma'][0] and self.inds[d]['pch'][0] < self.inds[d]['llpc'][0]

            if self.getposition(data=d).size < self.params.trade_size and buy_cond :
                self.inds[d]['order'] = self.buy(data=d,price=self.inds[d]['pch'][0],exectype=bt.Order.Stop)
