...
class LTF6(bt.Strategy):
    ...
    def __init__(self):
        ...
        for d in self.datas :
            ...
            self.inds[d]['channel'] = dmi_inds.BCH(d,subplot=False)
            self.inds[d]['bbh'] = self.inds[d]['channel'].bbh
            self.inds[d]['rbl'] = self.inds[d]['channel'].rbl
    ...
    def next(self):
        ...
        for d in self.d_with_len:
            ...
            if self.getposition(data=d).size < self.params.trade_size and self.inds[d]['bbh'] > 0:
                self.inds[d]['order'] = self.buy(data=d, price=self.inds[d]['bbh'][0], exectype=bt.Order.Stop)
                # print('buy condition met')

            if self.getposition(data=d).size > 0 and self.inds[d]['rbl'] > 0:
                self.inds[d]['order'] = self.close(data=d, price=self.inds[d]['rbl'][0], exectype=bt.Order.Stop)
