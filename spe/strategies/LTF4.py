...
class LTF4(bt.Strategy):
    ...
    def __init__(self):
        ...
        for d in self.datas :
            ...
            self.inds[d]['lllpc'] = self.inds[d]['hhhchannel'].lllpc
    ...
    def next(self):
        ...
        for d in self.d_with_len:
            ...
            if self.getposition(data=d).size > 0 and self.inds[d]['lllpc'] > 0 :
                self.inds[d]['order'] = self.close(data=d, price=self.inds[d]['lllpc'][0], exectype=bt.Order.Stop)
