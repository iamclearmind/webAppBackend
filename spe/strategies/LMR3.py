...
class LMR3(bt.Strategy):
    ...
    def next(self):
        ...
        for d in self.d_with_len:
            ...
            if self.getposition(data=d).size < self.params.trade_size and d.close[0] > self.inds[d]['csma200'][0] and d.high[0] < self.inds[d]['llpc'][0]  :
            ...
            if self.getposition(data=d).size > 0  :
                self.inds[d]['order'] = self.close(data=d,price=self.inds[d]['pcl'][0] * 0.9990, exectype=bt.Order.Stop)
