...
class LTF12B1(bt.Strategy):
    ...
    def notify_trade(self, trade):
        self.inds[trade.data]['last_buy_price'] *= 1.05

    def prenext(self):
        # Populate d_with_len
        self.d_with_len = [d for d in self.datas if len(d) > 0]  # changed value to 0
    ...
    def next(self):
        ...
        for d in self.d_with_len:
            ...
            # if(self.getposition(data=d).size > 5):
            # print("Not Okay: "+str(self.getposition(d).size)+ " "+str(d._name))
            # self.inds[d]['temp'].append(self.getposition(d).size)
            if (d.datetime.datetime() == datetime.datetime(2000, 5, 15)):
                print("Here")

            if self.getposition(data=d).size < self.params.trade_size:
                if self.inds[d]['first_order']:
                    self.inds[d]['buy_order'] = self.buy(data=d, price=self.inds[d]['newHigh'][0],
                                                         exectype=bt.Order.Stop)
                    self.inds[d]['last_buy_price'] = self.inds[d]['newHigh'][0]
                    self.inds[d]['first_order'] = False
                else:
                    self.inds[d]['buy_order'] = self.buy(data=d, price=self.inds[d]['last_buy_price'] * 1.10,
                                                         exectype=bt.Order.Stop)
            ...
            if self.getposition(data=d).size > 0:
                self.inds[d]['sell_order'] = self.close(data=d, price=self.inds[d]['newHighStop80'][0],
                                                        exectype=bt.Order.Stop)
                self.inds[d]['first_order'] = True
                # self.inds[d]['']
