...
class LTF1(bt.Strategy):
    ...
    def __init__(self):
        ...
        for d in self.datas :
            ...
            self.inds[d]['channel'] = MHLHL.MHLHL(self.data0,subplot=False)
            self.inds[d]['mhh'] = self.inds[d]['channel'].MHH
            self.inds[d]['mll'] = self.inds[d]['channel'].MLL
            # self.channel = MHLHL.MHLHL(self.data0,subplot=False)
            # self.mhh = self.channel.MHH
            # self.mll = self.channel.MLL
    ...
    def next(self):
        ...
        for d in self.d_with_len :
            ...
            if self.getposition(data=d).size < self.params.trade_size :
                self.inds[d]['order'] = self.buy(data=d, price=self.inds[d]['mhh'][0], exectype=bt.Order.Stop)
                # print('buy condition met')

            if self.getposition(data=d).size > 0 :
                self.inds[d]['order'] = self.close(data=d, price=self.inds[d]['mll'][0], exectype=bt.Order.Stop)
