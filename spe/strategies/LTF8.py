...
class LTF8(bt.Strategy):
    ...
    def __init__(self):
        ...
        for d in self.datas :
            ...
            self.inds[d]['MHH'] = self.inds[d]['mhlhl'].MHH
            self.inds[d]['MHL'] = self.inds[d]['mhlhl'].MHL
    ...
    def next(self):
        ...
        for d in self.d_with_len:
            ...
            if self.getposition(data=d).size < self.params.trade_size and d.close[0] > self.inds[d]['MHH'][0]  :
                self.inds[d]['order'] = self.buy(data=d,price=d.open[0], exectype=bt.Order.Stop)
                # print('buy condition met')

            if self.getposition(data=d).size > 0 and d.close[0] < self.inds[d]['MHL'][0] :
