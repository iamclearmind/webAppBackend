...
class LTF12A1(bt.Strategy):
    ...
    def notify_trade(self, trade):
        self.inds[trade.data]['last_buy_price'] *= 1.10
        ...
        for d in self.d_with_len:
            ...
            if self.getposition(data=d).size < self.params.trade_size  :
                if self.inds[d]['first_order'] :
                    ...
                    self.inds[d]['buy_order'] = self.buy(data=d,price=self.inds[d]['last_buy_price']*1.10,exectype=bt.Order.Stop)
            ...
            if self.getposition(data=d).size > 0 :
                self.inds[d]['sell_order'] = self.close(data=d,price=self.inds[d]['newHighStop75'][0],exectype=bt.Order.Stop)
                self.inds[d]['first_order'] = True
                # self.inds[d]['']
