...
class LTF16(bt.Strategy):
    ...
    def next(self):
        ...
        for d in self.d_with_len:
            ...
            if self.getposition(data=d).size < self.params.trade_size and self.inds[d]['PDIH'][0] > 0 and self.inds[d]['MDIL'][0] > 0:
                self.inds[d]['order'] = self.buy(data=d,price=self.inds[d]['PDIH'][0], exectype=bt.Order.Stop)
                # print('buy condition met')

            if self.getposition(data=d).size > 0 and d.close[0] < self.inds[d]['MDIL'][0] :
                self.inds[d]['order'] = self.close(data=d,price=self.inds[d]['MDIL'][0] ,exectype=bt.Order.Stop)
