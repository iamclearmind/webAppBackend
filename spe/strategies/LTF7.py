...
class LTF7(bt.Strategy):
    ...
    def next(self):
        ...
        for d in self.d_with_len:
            ...
            if self.getposition(data=d).size < self.params.trade_size and d.close[0] > self.inds[d]['csma200'][0] :
                self.inds[d]['order'] = self.buy(data=d,price=d.open[0], exectype=bt.Order.Stop)
                # print('buy condition met')

            if self.getposition(data=d).size > 0 and d.close[0] < self.inds[d]['csma200'][0] :
