...
class LTF12B(bt.Strategy):
    ...
    def next(self):
        ...
        for d in self.d_with_len:
            ...
            if self.getposition(data=d).size > 0 :
                self.inds[d]['order'] = self.close(data=d,price=self.inds[d]['newHighStop80'][0],exectype=bt.Order.Stop)
            ...
            #     hi = self.data0.high.get(size=self.inds[d]['counter'])
            #     lo = self.data0.low.get(size=self.inds[d]['counter'])
            #     self.inds[d]['eligible'] = False
