...
class FFM(bt.Strategy):
    ...
    def __init__(self):
        ...
        # self.save_date = datetime.datetime(2020, 6, 1) #(Year,Month,Day)
        # self.scrips = self.datas[:]
    ...
    def stop(self):
        ...
        # self.d_with_len = [d for d in self.scrips if len(d) > 60]
        ...
        for i, d in enumerate(self.datas):
            ...
            print(f'Date for data {dn} @ {d.num2date()}')

            # if d.num2date() == self.save_date:
            #
            #     print(f'Date condition met for data {dn} @ {d.num2date()}')

            self.last_values['Date'] = d.num2date()
            ...
            dataclose = d.close[0]
                ...
                # self.last_values['RSLT'] = 1 if self.inds[d]['RSCMA10'] > self.inds[d][
                #     'RSCMA200'] else 0  # New addition
            self.last_values['RS'] = 1 if self.inds[d]['rsch'].rsc[0] > self.inds[d]['RSCMA10'][0] else 0
            ...
            self.last_values['NH'] = 1 if self.inds[d]['newHigh'].newHigh[0] > self.inds[d]['newHigh'].newHigh[-1] else 0
            self.last_values['CHH'] = 1 if dataclose > self.inds[d]['HPCHU'].l.hpchu[0] else 0

                # print('current new high ' + str(self.inds[d]['newHigh'][0]))
                # print('previous new high ' + str(self.inds[d]['newHigh'][-10]))
                # print('previous new high ' + str(self.inds[d]['newHigh'][-20]))
                # print('previous new high ' + str(self.inds[d]['newHigh'][-30]))
                # print('previous new high ' + str(self.inds[d]['newHigh'][-40]))
                # print('previous new high ' + str(self.inds[d]['newHigh'][-50]))
                # print('previous new high ' + str(self.inds[d]['newHigh'][-60]))
                ...
                # self.last_values['%>HHC'] = (dataclose - self.inds[d]['pc'].hhpc[0]) / self.inds[d]['pc'].hhpc[
                #     0] * 100 if dataclose > self.inds[d]['pc'].hhpc[0] else 0
                # self.last_values['%<LLC'] = (dataclose - self.inds[d]['pc'].llpc[0]) / self.inds[d]['pc'].llpc[
                #     0] * 100 if dataclose < self.inds[d]['pc'].llpc[0] else 0
                #
